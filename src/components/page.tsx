...
const GenerateProgramPage = () => {
  ...
  const [collectedVars, setCollectedVars] = useState<any>({});
  ...
  // Suppress "Meeting has ended" console noise
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleVariableUpdate = (vars: Record<string, any>) => {
      console.log("🧩 Variables updated:", vars);
      setCollectedVars((prev: any) => ({ ...prev, ...vars }));
    };
  ...
  try {
    ...
    console.error("❌ Error during handleCallEnd:", err);
  }
};
...
// 👇 Optionally listen manually if needed
;(vapi as any).on("variables-updated", handleVariableUpdate);

    return () => {
      vapi
        .off("call-start", handleCallStart)
        .off("variables-updated", handleVariableUpdate)
  ...
  }, [sessionId, user?.id, collectedVars]);
